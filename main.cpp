...
void inicializar_sistema()
{
    ...
    // First, assign pages for each task
    auxjt = pjt;
    while (auxjt != NULL)
    {
    ...
    // Assign pages with sequence
    asigna_paginas_con_secuencia();

    // Create PCB entries for each assigned page in PMT
    for(int i = 0; i < N; i++) // For each task
    {
        PMT *auxp = pp[i];
        while(auxp != NULL)
        {
            // If this page has a frame assigned
            if(auxp->estado == 1 && auxp->marco != 0)
            {
                nuevopcb = new PCB;
                memset(nuevopcb, 0, sizeof(PCB));
                
                // Create process name based on task number and page number
                sprintf(nuevopcb->proceso, "J%dP%d", i + 1, auxp->pagina);
...
void imprime_PMT()
{
    for (int i = 0; i < N; i++)
    {
        ...
        // Check if this PMT has any assigned frames
        PMT *auxp = pp[i];
        ...
        // Only print PMT if it has assigned frames
        if (hasFrames)
        {
            cout << "\n=== PMT de la tarea " << i + 1 << " ===\n";

            // Print sequence for this task
            JT *auxjt = pjt;
            ...
            // Only print pages that have assigned frames
            auxp = pp[i];
            while (auxp != NULL)
            {
                if (auxp->estado == 1 && auxp->marco != 0)
